      ...
      >
        <CardGrid>
          <Card label="Apa itu" type="what">
            <p>React Testing Library (RTL) adalah library testing yang mendorong kamu test komponen seperti yang user lakukan (mencari teks, klik tombol), bukan implementasi internal. Vitest adalah test runner modern yang cepat, cocok dipakai bersama Vite.</p>
          </Card>
          <Card label="Kenapa penting" type="why">
            <p>Testing adalah skill yang membedakan developer junior yang baik. Di interview sering ditanyakan. Di pekerjaan, kemampuan menulis test yang baik membuat kamu dipercaya untuk mengerjakan fitur yang lebih besar.</p>
          </Card>
        ...
        <MentalModel label="Filosofi Utama RTL">
          <p>"Test what the user sees, not how it works internally." Artinya: jangan test nama state variable atau nama fungsi internal. Test apakah teks yang benar muncul di layar, apakah tombol bisa diklik, apakah form menampilkan error yang benar.</p>
        </MentalModel>

        <CodeBlock lang="bash" file="terminal - instalasi vitest rtl user-event dan jest-dom" id="test-setup-install" html={`npm install -D vitest @testing-library/react @testing-library/user-event @testing-library/jest-dom jsdom`} />

        <CodeBlock lang="jsx" file="vite.config.js - tambahkan konfigurasi test" id="test-setup-vite" html={`<span class="kw">import</span> { defineConfig } <span class="kw">from</span> <span class="str">'vite'</span>;
...
<span class="kw">export default</span> <span class="fn">defineConfig</span>({
  ...
  test: {
    ...
    setupFiles: <span class="str">'./src/setupTests.js'</span>,
  },
});`} />

        <CodeBlock lang="jsx" file="src/setupTests.js - setup jest-dom matchers" id="test-setup-setup" html={`<span class="kw">import</span> <span class="str">'@testing-library/jest-dom'</span>;
<span class="cmt">// Ini menambahkan matcher seperti .toBeInTheDocument(), .toBeVisible(), dll.</span>`} />
      ...
      >
        <CardGrid>
          <Card label="Apa itu" type="what">
            <p><code>render()</code> merender komponen ke DOM virtual. Query functions seperti <code>getByRole</code>, <code>getByText</code>, <code>getByLabelText</code> digunakan untuk mencari elemen di DOM tersebut.</p>
          </Card>
          <Card label="Kenapa penting" type="why">
            <p>Urutan prioritas query adalah kunci menulis test yang baik. Menggunakan query yang tepat membuat test lebih resilient dan lebih mencerminkan pengalaman user sesungguhnya.</p>
          </Card>
          <Card label="Kapan dipakai" type="when">
            <ul>
              <li>Setiap test komponen dimulai dengan <code>render()</code></li>
              <li>Gunakan <code>getByRole</code> sebagai prioritas pertama</li>
              <li>Gunakan <code>getByText</code> untuk konten teks</li>
              <li>Hindari <code>getByTestId</code> kecuali tidak ada pilihan lain</li>
            </ul>
          </Card>
        </CardGrid>

        <CodeBlock lang="jsx" file="src/__tests__/TaskCard.test.jsx - unit test render dan kondisi komponen TaskCard" id="test-render-taskcard" html={`<span class="kw">import</span> { render, screen } <span class="kw">from</span> <span class="str">'@testing-library/react'</span>;
<span class="kw">import</span> TaskCard <span class="kw">from</span> <span class="str">'../components/TaskCard'</span>;

<span class="kw">const</span> mockTask = {
...
<span class="fn">describe</span>(<span class="str">'TaskCard'</span>, () => {
  <span class="fn">it</span>(<span class="str">'menampilkan judul task dengan benar'</span>, () => {
    <span class="fn">render</span>(<span class="tag">&lt;TaskCard</span> <span class="atr">task</span>=<span class="jsx">{mockTask}</span> <span class="tag">/&gt;</span>);

    <span class="cmt">// getByText: cari elemen yang mengandung teks ini</span>
  ...
  <span class="fn">it</span>(<span class="str">'menampilkan badge prioritas tinggi'</span>, () => {
    ...
    <span class="cmt">// getByRole: cara terbaik untuk mencari elemen berdasarkan aksesibilitas</span>
  ...
  <span class="fn">it</span>(<span class="str">'tidak menampilkan tombol edit ketika isOwner = false'</span>, () => {
    <span class="fn">render</span>(<span class="tag">&lt;TaskCard</span> <span class="atr">task</span>=<span class="jsx">{mockTask}</span> <span class="atr">isOwner</span>=<span class="jsx">{false}</span> <span class="tag">/&gt;</span>);

    <span class="cmt">// queryByRole: tidak throw error jika tidak ditemukan (berbeda dari getByRole)</span>
    <span class="kw">expect</span>(screen.<span class="fn">queryByRole</span>(<span class="str">'button'</span>, { name: /edit/i })).<span class="fn">not</span>.<span class="fn">toBeInTheDocument</span>();
        ...
        <CodeBlock lang="jsx" file="referensi - urutan prioritas query selector RTL" id="test-render-priority" html={`<span class="cmt">// Prioritas tinggi ke rendah (gunakan dari atas ke bawah):</span>
...
<span class="cmt">// findBy   - async, return promise (untuk elemen yang muncul setelah loading)</span>`} />
      ...
      >
        <CardGrid>
          <Card label="Apa itu" type="what">
            <p><code>userEvent</code> dari <code>@testing-library/user-event</code> mensimulasikan interaksi user yang lebih realistis dibanding <code>fireEvent</code>. <code>waitFor</code> dan <code>findBy*</code> digunakan untuk menunggu perubahan DOM yang async.</p>
          </Card>
          <Card label="Kenapa penting" type="why">
            <p>Form, tombol, dan elemen interaktif lainnya adalah bagian yang paling sering perlu ditest. <code>userEvent</code> lebih realistis karena mensimulasikan events yang sebenarnya terjadi saat user berinteraksi.</p>
          </Card>
          <Card label="Kapan dipakai" type="when">
            <ul>
        ...
        <CodeBlock lang="jsx" file="src/__tests__/TaskForm.test.jsx - test form dan validasi" id="test-events-form" html={`<span class="kw">import</span> { render, screen, waitFor } <span class="kw">from</span> <span class="str">'@testing-library/react'</span>;
...
<span class="fn">describe</span>(<span class="str">'TaskForm'</span>, () => {
  <span class="fn">it</span>(<span class="str">'menampilkan error validasi ketika judul kurang dari 3 karakter'</span>, <span class="kw">async</span> () => {
    <span class="kw">const</span> user = userEvent.<span class="fn">setup</span>(); <span class="cmt">// buat instance user event</span>
    <span class="fn">render</span>(<span class="tag">&lt;TaskForm</span> <span class="atr">onClose</span>=<span class="jsx">{vi.fn()}</span> <span class="tag">/&gt;</span>);

    <span class="cmt">// 1. Ketik judul yang terlalu pendek</span>
    ...
    <span class="cmt">// 2. Klik tombol submit</span>
    ...
    <span class="cmt">// 3. Tunggu dan cek apakah pesan error muncul</span>
  ...
  <span class="fn">it</span>(<span class="str">'memanggil onClose setelah form berhasil disubmit'</span>, <span class="kw">async</span> () => {
    ...
    <span class="kw">const</span> mockOnClose = vi.<span class="fn">fn</span>(); <span class="cmt">// mock function untuk cek apakah dipanggil</span>

    <span class="fn">render</span>(<span class="tag">&lt;TaskForm</span> <span class="atr">onClose</span>=<span class="jsx">{mockOnClose}</span> <span class="tag">/&gt;</span>);
    ...
    <span class="kw">await</span> <span class="fn">waitFor</span>(() => {
      <span class="kw">expect</span>(mockOnClose).<span class="fn">toHaveBeenCalledOnce</span>();
        ...
        <MistakeBlock>
          <li>Menggunakan <code>fireEvent.click()</code> alih-alih <code>userEvent.click()</code> - fireEvent lebih rendah level dan tidak mensimulasikan urutan event yang sebenarnya (mousedown, mouseup, click)</li>
          <li>Lupa <code>await</code> sebelum <code>userEvent.type()</code> dan <code>userEvent.click()</code> - user-event v14 semuanya async</li>
          <li>Menggunakan <code>getBy</code> untuk elemen yang muncul secara async - gunakan <code>findBy</code> yang mengembalikan promise dan menunggu elemen muncul</li>
        </MistakeBlock>
      </TopicSection>
      ...
      >
        <CardGrid>
          <Card label="Apa itu" type="what">
            <p>Mocking adalah teknik menggantikan dependency nyata (seperti API call) dengan versi palsu yang bisa kita kontrol hasilnya. Ini memastikan test tidak bergantung pada network atau server eksternal.</p>
          </Card>
          <Card label="Kenapa penting" type="why">
            <p>Test yang memanggil API nyata itu lambat, tidak reliable, dan bisa gagal karena masalah network. Mock membuat test cepat, terisolasi, dan deterministik - hasilnya selalu sama setiap kali dijalankan.</p>
          </Card>
          <Card label="Kapan dipakai" type="when">
            <ul>
        ...
        <CodeBlock lang="jsx" file="src/__tests__/DashboardPage.test.jsx - mock API dengan QueryClientProvider" id="test-mock-code" html={`<span class="kw">import</span> { render, screen } <span class="kw">from</span> <span class="str">'@testing-library/react'</span>;
...
<span class="cmt">// Mock seluruh module taskApi</span>
vi.<span class="fn">mock</span>(<span class="str">'../api/taskApi'</span>);

<span class="cmt">// Helper untuk membungkus komponen dengan semua Provider yang diperlukan</span>
<span class="kw">const</span> <span class="fn">renderWithProviders</span> = (component) => {
  <span class="kw">const</span> queryClient = <span class="kw">new</span> <span class="fn">QueryClient</span>({
    defaultOptions: { queries: { retry: <span class="kw">false</span> } }, <span class="cmt">// tidak retry di test</span>
  });
  <span class="kw">return</span> <span class="fn">render</span>(
    <span class="tag">&lt;QueryClientProvider</span> <span class="atr">client</span>=<span class="jsx">{queryClient}</span><span class="tag">&gt;</span>
      <span class="tag">&lt;BrowserRouter&gt;</span>{component}<span class="tag">&lt;/BrowserRouter&gt;</span>
...
<span class="fn">describe</span>(<span class="str">'DashboardPage'</span>, () => {
  <span class="fn">it</span>(<span class="str">'menampilkan daftar task setelah data berhasil di-fetch'</span>, <span class="kw">async</span> () => {
    <span class="cmt">// Atur apa yang dikembalikan mock</span>
    taskApi.fetchTasks.<span class="fn">mockResolvedValue</span>([
      { id: <span class="num">1</span>, title: <span class="str">'Task Pertama'</span>, status: <span class="str">'todo'</span> },
      { id: <span class="num">2</span>, title: <span class="str">'Task Kedua'</span>, status: <span class="str">'done'</span> },
    ]);

    <span class="fn">renderWithProviders</span>(<span class="tag">&lt;DashboardPage /&gt;</span>);

    <span class="cmt">// findByText = async, menunggu elemen muncul setelah loading selesai</span>
  ...
  <span class="fn">it</span>(<span class="str">'menampilkan pesan error ketika API gagal'</span>, <span class="kw">async</span> () => {
    <span class="cmt">// Simulasi API error</span>
    taskApi.fetchTasks.<span class="fn">mockRejectedValue</span>(<span class="kw">new</span> <span class="fn">Error</span>(<span class="str">'Network Error'</span>));
...
<span class="cmt">// afterEach  = setelah setiap test</span>`} />
        ...
        <MistakeBlock>
          <li>Tidak membungkus komponen dengan Provider yang diperlukan (QueryClientProvider, Redux Provider) saat rendering di test - ini akan menyebabkan error karena hooks tidak bisa menemukan context</li>
          <li>Menggunakan <code>getByText</code> untuk konten yang muncul secara async (setelah loading) - gunakan <code>await findByText()</code> untuk menunggu konten muncul</li>
          <li>Tidak mereset mock antar test dengan <code>vi.clearAllMocks()</code> di <code>afterEach</code> - sisa mock dari test sebelumnya bisa mempengaruhi test berikutnya</li>
        </MistakeBlock>
      </TopicSection>
    </>
  );
